...
-- URL: /profile
INSERT INTO enrollment (userId, classId, enrollmentDate)
VALUES (?, ?, ?);
...
INSERT INTO availability (userId, studyDate, startTime, endTime)
VALUES (?, ?, ?, ?);
...
-- URL: /profile/availability
SELECT availabilityId, userId, studyDate, startTime, endTime
FROM availability
WHERE userId = ?
ORDER BY studyDate, startTime;

-- Gets all availabilities for users that a given user follows. Used for the home page's main functionality.
-- URL: /
SELECT u.userId, u.firstName, u.lastName, a.availabilityId, a.studyDate, a.startTime, a.endTime
FROM user u
         JOIN follow f ON u.userId = f.followeeId
...
-- URL: /
INSERT INTO study_request (requesterId, availabilityId, requestDate)
VALUES (?, ?, ?);
...
SELECT sr.requestId,
       ...
       sr.status,
       sr.requestDate,
       u1.firstName AS requesterFirstName,
       u1.lastName  AS requesterLastName,
...
WHERE (sr.requesterId = ? OR t.userId = ?)
  AND sr.status = 'APPROVED'
  AND a.studyDate >= CURRENT_DATE
